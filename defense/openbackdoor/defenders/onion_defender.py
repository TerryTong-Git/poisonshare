        ...
        for instance in tqdm(poison_data):
            ...
            for message in instance["messages"]:
                if message["role"] == "user" and len(message["content"].split()) > 1:
                    process_text = self.get_processed_text(self,orig_text=message["content"], bar=self.threshold)
                    message["content"] = process_text
                filtered_messages.append(message)
            instance["messages"] = filtered_messages
            process_data_li.append(instance)
            
        print('\n'*2)
        print('finish onion defend')
        print('\n'*2)
        return datasets.Dataset.from_list(process_data_li)
...
class GPT2LM():
    def __init__(self, parallel):
        ...
        self.lm = transformers.GPT2LMHeadModel.from_pretrained("gpt2",load_in_4bit=False, torch_dtype= torch.float16,attn_implementation="flash_attention_2").to(self.device)
        if parallel:
