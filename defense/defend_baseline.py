...
def main():
    if "bki":
        ...
        model, tokenizer = load_model(model_path, AutoModelForCausalLM)
        clean = bki.correct(bki, poison_data=poison_dataset,
                            model=model, model_path=model_path)
        clean.save_to_disk(
            "/nas04/terry69/poison_data/working/rare/cleaned_bki_20.0p/train")
    else:
        onion = load_defender("onion")
        poison_dataset = load_ds("full")
        clean = onion.correct(onion, poison_data=poison_dataset)
        clean.save_to_disk(
            "/nas03/terry69/poison_data/working/rare/clean_onion_10.0p/train")
