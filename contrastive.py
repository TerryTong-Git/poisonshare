...
class DoLa:
    ...
    def load_model(self, model_name):
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            model_name if not 'vicuna' in model_name else 'huggyllama/llama-7b')
    ...
    def set_stop_words(self, stop_words):
        ...
        for stop_word in self.stop_words:
            ...
            print("Added stop word: ", stop_word,
                  'with the ids', stop_word_ids, flush=True)
        ...
        with torch.no_grad():
            ...
            if mode == 'baseline':
                ...
                log_probs = outputs[range(
                    outputs.shape[0]), continue_ids].sum().item()
                ...
                log_probs = diff_logits[range(
                    diff_logits.shape[0]), continue_ids].sum().item()

            elif mode == 'dola':
                premature_layer_dist = {
                    l: 0 for l in candidate_premature_layers}
                picked_logits = []
                result_dict = {}
                premature_layers = []
                ...
                for seq_i in range(prefix_ids.shape[-1] - 1, input_ids.shape[-1] - 1):
                    ...
                    log_softmax_mature_layer = F.log_softmax(
                        dict_outputs[mature_layer][:, seq_i, :], dim=-1)  # shape: (batch_size, num_features)
                    # shape: (num_premature_layers, batch_size, num_features)
                    ...
                    # shape: (num_premature_layers, batch_size)
                    kl2 = F.kl_div(log_softmax_premature_layers,
